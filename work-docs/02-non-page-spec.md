...
- 実装は「カウンターのユースケース」と「保存先アダプタ（KV / 代替実装）」を分離する
  - 将来 Kotlin API や別ストアへ移行する際、境界を差し替えやすくする
  - ローカル開発では Cloudflare KV に書かず、インメモリ実装で再現できるようにする
...
- フロント実装は `fetch` 直書きを避け、送受信のゲートウェイ境界（interface）を通して呼び出す
  - `POST` が失敗した場合は `GET` で読み取りにフォールバックし、UI が `—` 固定にならないようにする
