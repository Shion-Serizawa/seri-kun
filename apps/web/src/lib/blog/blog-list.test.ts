...
describe('sortBlogListNewestFirst', () => {
  it('sorts by publishedAt descending', () => {
    const items = [
      { slug: 'a', title: 'A', publishedAt: '2026-01-01', tags: [], description: 'd' },
      { slug: 'b', title: 'B', publishedAt: '2026-02-01', tags: [], description: 'd' },
      { slug: 'c', title: 'C', publishedAt: '2025-12-31', tags: [], description: 'd' },
    ];

    const sorted = sortBlogListNewestFirst(items);
    expect(sorted.map((x) => x.slug)).toEqual(['b', 'a', 'c']);
  });

  it('uses title as a secondary sort when dates are identical', () => {
    const items = [
      { slug: 'b', title: 'b', publishedAt: '2026-01-01', tags: [], description: 'd' },
      { slug: 'a', title: 'a', publishedAt: '2026-01-01', tags: [], description: 'd' },
      { slug: 'c', title: 'c', publishedAt: '2026-01-02', tags: [], description: 'd' },
    ];

    const sorted = sortBlogListNewestFirst(items);
    expect(sorted.map((x) => x.slug)).toEqual(['c', 'a', 'b']);
  });
});
...
describe('groupBlogPostsByMonth', () => {
  it('buckets posts by toMonthKey(publishedAt) and sorts months descending', () => {
    const items = [
      { slug: 'jan-a', title: 'A', publishedAt: '2026-01-10', tags: [], description: 'd' },
      { slug: 'feb', title: 'B', publishedAt: '2026-02-01', tags: [], description: 'd' },
      { slug: 'jan-b', title: 'C', publishedAt: '2026-01-01', tags: [], description: 'd' },
      { slug: 'bad', title: 'D', publishedAt: '2026/03/01', tags: [], description: 'd' },
    ];
    ...
    expect(jan?.posts.map((p) => p.slug)).toEqual(['jan-a', 'jan-b']);
  });
});
...
describe('listBlogTags', () => {
  it('extracts unique tags and sorts alphabetically', () => {
    const items = [
      { slug: 'a', title: 'A', publishedAt: '2026-01-01', tags: ['z', 'a'], description: 'd' },
      { slug: 'b', title: 'B', publishedAt: '2026-01-02', tags: ['a', 'm'], description: 'd' },
      { slug: 'c', title: 'C', publishedAt: '2026-01-03', tags: [], description: 'd' },
    ];
